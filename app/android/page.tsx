...
import { Smartphone, Upload, Trash2, Play } from "lucide-react";
import type { ImportedApp } from "@/types/ui";

// Mock imported apps - in production these would come from Telegram/Discord storage
...
export default function AndroidPage() {
  ...
  const handleImport = async () => {
    ...
    const newApp: ImportedApp = {
      ...
      name: `App ${importedApps.length + 1}`,
      type: "android",
      size: Math.floor(Math.random() * 50000000) + 10000000,
      uploadedAt: new Date(),
      storageRef: `telegram:android_${Date.now()}`,
    };
        ...
        >
          ...
          <div className="text-center py-12">
            ...
            <p className="text-white/50 text-lg font-light max-w-2xl mx-auto">
              Import and run Android applications directly in your browser. 
              Your apps are stored securely in cloud storage.
            </p>
          </div>
          ...
          >
            <GlowingCard className="p-6">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  <h3 className="text-white/90 font-medium mb-1">Import APK</h3>
                  <p className="text-white/50 text-sm">
                    Upload Android packages to run them instantly
                  </p>
                </div>
                <GradientButton
                  variant="primary"
          ...
          >
            <h2 className="text-white/70 text-sm uppercase tracking-wider mb-4">
              Imported Apps ({importedApps.length})
            </h2>
            ...
            {importedApps.length === 0 && (
              <div className="text-center py-12 text-white/40">
                ...
                <p>No apps imported yet</p>
                <p className="text-sm">Import an APK to get started</p>
              </div>
            )}
          </motion.div>
        </motion.div>
