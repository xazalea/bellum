...
export function ExeRunner() {
  ...
  const loaderRef = useRef<{ stop: () => void } | null>(null);
  ...
  const handleRun = useCallback(async () => {
    ...
    setStatusDetail('Loading Windows runtime...');

    try {
      const { X86Loader } = await import('@/lib/engine/loaders/x86-loader');
      const loader = new X86Loader();
      ...
      setStatus('loading');
      setStatusDetail('Translating x86 binary to WebAssembly...');
      ...
      await loader.load(containerRef.current, objectUrl);

      setStatus('running');
      setStatusDetail(file.name);
      ...
      {!isRunning && (
        ...
        >
          ...
          <p className="text-sm font-medium mb-1">
            {file ? file.name : 'Drop EXE file here or click to browse'}
          </p>
          <p className="text-xs text-muted-foreground mb-3">
            {file ? `${(file.size / 1024 / 1024).toFixed(1)} MB` : 'Supports .exe files'}
          </p>
          <Input
            type="file"
            accept=".exe"
      ...
      >
        {!isRunning && (
          <div className="absolute inset-0 flex items-center justify-center">
            <p className="text-sm text-white/30">EXE output will appear here</p>
          </div>
        )}
      </div>
    </div>
