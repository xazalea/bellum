  ...
  ({ className, onMouseEnter, onMouseLeave, ...props }, ref) => {
    ...
    const handleMouseEnter = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => {
      if (resolvedRef.current) {
        safeAnimate(resolvedRef.current, presets.cardHover());
      }
      onMouseEnter?.(e);
    }, [onMouseEnter, resolvedRef]);

    const handleMouseLeave = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => {
      if (resolvedRef.current) {
        safeAnimate(resolvedRef.current, presets.cardUnhover());
      }
      onMouseLeave?.(e);
    }, [onMouseLeave, resolvedRef]);

    return (
      <div
        ref={resolvedRef}
        className={cn(
          'rounded-lg border bg-card text-card-foreground shadow-matte hover-lift',
          className
        )}
