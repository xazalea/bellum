...
export class WindowsVM extends BaseVM {
  ...
  async start(): Promise<void> {
    ...
    // Get Windows bundle URL from Puter.js storage
    let bundleUrl: string;
    try {
      const bundlePath = `${this.state.storagePath}/windows_bundle.jsdos`;
      try {
        await puterClient.getFileInfo(bundlePath);
        bundleUrl = await puterClient.getReadURL(bundlePath);
      } catch (error) {
        throw new Error('Windows bundle not found. Please upload a Windows bundle file.');
      }
    } catch (error) {
      console.error('Failed to load Windows bundle:', error);
      throw error;
    }

    // Try to restore saved state if available
    try {
      const statePath = `${this.state.storagePath}/windows_state.bin`;
  ...
  // Override saveState to also save Windows state
