/**
 * Challenger Loader - Orchestrates the Transpiler Pipeline
...
import { hypervisor } from '../../challenger/core/hypervisor';
import { neuralAccelerator } from '../../challenger/gpu/transformer';
import { cpuManager } from '../../challenger/cpu/cpu-manager';
import JSZip from 'jszip';

export class ChallengerLoader {
  ...
  async load(container: HTMLElement, filePath: string, type: FileType) {
    ...
    console.log(`Challenger Transpiler: Loading ${filePath} as ${type}`);
  ...
  private async instantiateAndRun(wasmBytes: Uint8Array) {
    ...
    try {
        ...
        if (exports.start) {
            console.log('Challenger: Executing Entry Point...');
            exports.start();
            ...
            console.warn('Challenger: No start function exported');
        }
