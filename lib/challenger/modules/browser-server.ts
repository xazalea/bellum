...
import { challengerEngine } from '../engine';
...
export class BrowserServerEngine {
    ...
    tempShareLinks = {
        generate: (resourceId: string, expiry: number) => {
            return `https://challenger.run/share/${resourceId}?exp=${expiry}`;
        }
    };
    ...
    localCdn = {
        ...
        init: async () => {
            this.localCdn.cache = await caches.open('challenger-cdn-v1');
        }
    };
    ...
    vncDesktopRunner = {
        ...
        start: async (image: string) => {
            ...
            if (!challengerEngine) throw new Error('Challenger engine is only available in the browser');
