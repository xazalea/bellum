...
export class DistributedComputeService {
    ...
    public async register(username: string): Promise<{ success: boolean; message: string }> {
        ...
        if (typeof window !== 'undefined') localStorage.setItem('challenger_username', username);
    ...
    public logout() {
        ...
        if (typeof window !== 'undefined') localStorage.removeItem('challenger_username');
    }
    ...
    public async linkDevice(code: string): Promise<boolean> {
        ...
        try {
            ...
            this.currentUser = account;
            localStorage.setItem('challenger_username', username);
            return true;
        } catch (e) {
    ...
    private loadRegistry() {
        try {
            if (typeof window === 'undefined') return;
            const stored = localStorage.getItem('challenger_cluster_registry');
            if (stored) {
    ...
    private persistRegistry() {
        ...
        if (typeof window !== 'undefined') localStorage.setItem('challenger_cluster_registry', JSON.stringify(obj));
    }

    private async restoreSession() {
        if (typeof window === 'undefined') return;
        const username = localStorage.getItem('challenger_username');
        if (username) {
