/**
 * Challenger Proxy Server
 * Local CORS proxy using Service Worker and Cache API
 * Similar architecture to v86 but scratch-made for Challenger Deep
 */

export class ChallengerProxyServer {
  ...
  private async initialize(): Promise<void> {
    if (typeof window === 'undefined' || !('serviceWorker' in navigator)) {
      console.warn('[ChallengerProxy] Service Workers not supported');
      return;
    }

    try {
      // Register the proxy service worker
      const registration = await navigator.serviceWorker.register('/challenger-proxy-sw.js', {
        scope: '/',
      });
      ...
      console.log('[ChallengerProxy] Service Worker initialized');
    } catch (error) {
      console.error('[ChallengerProxy] Failed to initialize:', error);
    }
  }
